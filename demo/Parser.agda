...
... | nothing = nothing
... | just (k , rest₃) with parseNatUntilColon rest₃
... | nothing = nothing
... | just (sd , rest₄) =
  just record
  { 
...
module GridProblem (n k th seed : ℕ)
  where
  ...
  runResult = C.run seed gridPropagator initialState
  ...
  renderRow w x =
    foldFin n
      (λ y acc →
        showMaybePattern
          (C.chooseFirstAllowed seed w (ι x ⊗ ι y))
        ++s acc)
      ""
...
runSpec spec =
  let
    ...
    th = threshold spec
    sd = seed spec
    module G = GridProblem n k th sd
  in
