...
--------------------------------------------------
    ...
    runStep : ℕ → Problem → State → StepResult
    runStep seed p w = do
      bool⇒res (noContradiction? w) w tt
      maybe⇒inv-res (nextMRVNode w) (Done ,′ w) λ i → do
        j ← maybe⇒res (chooseFirstAllowed seed w i) w
        let w′ = propagate p (observe i j w)
        bool⇒res (noContradiction? w′) w′ (Continue ,′ w′)

    runLimit : (limit : ℕ) → ℕ → Problem → State → RunResult
    runLimit 0 seed p w = err (OffLimit , w)
    runLimit (suc l) seed p w = do
      r , w′ ← (Fail ,′_) <$>ₑ runStep seed p w
      case r of λ where
        Done → ok w′
        Continue → runLimit l (nextSeed seed) p w′

    run : ℕ → Problem → State → RunResult
    run seed = runLimit (suc (shapeSize Cell)) seed
